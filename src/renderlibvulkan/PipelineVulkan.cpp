...
void PipelineVulkan::CreateGraphicPipeline(const DeviceVulkan& deviceVulkan,const RenderpassVulkan& renderpassVulkan)
{
    ...
    if (vkCreateDescriptorSetLayout(deviceVulkan.GetDevice(), &layoutInfo, nullptr, &m_DescriptorSetLayout) != VK_SUCCESS) {
    ...
    if (vkCreateGraphicsPipelines(deviceVulkan.GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_Pipeline) != VK_SUCCESS) {
